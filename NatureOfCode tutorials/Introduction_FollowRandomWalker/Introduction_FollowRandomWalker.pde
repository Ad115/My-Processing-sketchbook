...
class Walker
{
  ...
  void step()
  {
    float stepx;
    float stepy;
    // Decide wether follow the mouse or make another random move
    float follow = random(1);
    if (follow < prob) // Follow the mouse
    {
      float dirx = (mouseX-x)/abs(mouseX-x); // Unit vector in direction of the mouse
      float diry = (mouseY-y)/abs(mouseY-y); 
      stepx = dirx * random(1);
      stepy = diry * random(1);
    }
    else 
