class ChemicalField
{
  ...
  ChemicalField(float cellSize, float diffusionConstant, SourceLocation sources)
  ...
  { 
    ...
    this.sources = getDistribution(sources, SOURCE);
    // Initialize grids
  ...
  Sources[][] getDistribution(SourceLocation sourcesDistribution, Sources sourceType)
  {
    Sources[][] distribution = new Sources[rows][cols];
    
    switch (sourcesDistribution)
    {
      case TOP:
      {
        for (int i=0; i<rows; i++)
