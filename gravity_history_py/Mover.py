class Mover(object):
    ...
    def draw(self):
        ...
        for closeMover in self.closeTo:
            line(self.position.x, self.position.y, closeMover.position.x, closeMover.position.y)
        self.closeTo = []
    ...
    def attractMover(self, mover, connectedness):
        attraction = PVector.sub(self.position, mover.position)
        
        r = attraction.mag()
        #if  r*mover.velocity.magSq() < self.mass*connectedness: # Relative escape velocity: (v_1)^2 = 2*M_2/r
        if (r*r) < 200*mover.velocity.magSq()*connectedness:
        #if r < connectedness*100:
            self.closeTo.append(mover)
        if r < self.size:
